...
class TestAgent(BaseAgent):
    ...
    def _get_all_actions_faster(self, state: new_env.OthelloState):
        actions = []
    ...
    def __call__(self, original_state: pre_env.OthelloState, faster_state: new_env.OthelloState) -> pre_env.OthelloAction:
...
def run():
    ...
    for game in range(100):
        ...
        faster_state = new_env.OthelloEnv().initialize_state()
        ...
        while not original_state.done:
            ...
            for agent in agents:
                ...
                faster_state = new_env.OthelloEnv().step(faster_state, agent.agent_id, action)
