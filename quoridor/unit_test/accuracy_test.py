...
class TestAgent(BaseAgent):
    ...
    def _get_all_actions_faster(self, state: new_env.QuoridorState):
        legal_actions_np = new_env.QuoridorEnv().legal_actions(state, self.agent_id)
        actions = []
        for action_type in [0, 1, 2]:
            for coordinate_x in range(new_env.QuoridorEnv.board_size):
                for coordinate_y in range(new_env.QuoridorEnv.board_size):
                    action = [action_type, coordinate_x, coordinate_y]
                    if legal_actions_np[action_type, coordinate_x, coordinate_y]:
                        actions.append(action)
        return actions

    def __call__(self, original_state: pre_env.QuoridorState, faster_state: new_env.QuoridorState) -> pre_env.QuoridorAction:
...
def run():
    ...
    for game in range(100):
        ...
        faster_state = new_env.QuoridorEnv().initialize_state()
        ...
        while not original_state.done:
            ...
            for agent in agents:
                ...
                faster_state = new_env.QuoridorEnv().step(faster_state, agent.agent_id, action)
